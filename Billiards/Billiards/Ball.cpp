	...
	for(unsigned int i=0;i<this->data.size();++i){
		for(unsigned int j=0;j<this->data.size();++j){
			...
			if(GetDist(this->data.at(i)->GetPos(), this->data.at(j)->GetPos()) < this->data.at(i)->GetBallSize()+this->data.at(j)->GetBallSize()){
				//反射方向を算出する
				float ref_rate = (1.f + 1.0f);	//反発率(今回は1.0f)
				...
				this->data.at(i)->SetPos(this->data.at(i)->GetPos()+this->data.at(i)->GetSpeed());
				this->data.at(j)->SetPos(this->data.at(j)->GetPos()+this->data.at(j)->GetSpeed());
			}
		}
	}
}
...
void Ball_data::Process(){
	...
	//壁
	res = dx.GetHitColl_Model_Lay(table_hnd, this->pos, this->pos+this->speed);
	if(!res.data.empty()){
		for(unsigned int i=0;i<res.data.size();++i){
			if(0.5f < fabs(res.data.at(i)->norm.x) || 0.5f < fabs(res.data.at(i)->norm.z)){
				this->speed = ReflectVector(this->speed, res.data.at(i)->norm)*0.04f;
				this->pos = res.data.at(i)->pos+this->speed;
				//break;
			}
		}
	}
	...
	this->pos += this->speed;
	this->pos.y -= this->g;
	this->mv_ang += VGet(R(this->speed.x), R(this->speed.y), R(this->speed.z));

	if(this->speed != VZero){
		SetTrgLim(&this->speed.x, 0.f, 0.0005f);
		SetTrgLim(&this->speed.y, 0.f, 0.0005f);
		SetTrgLim(&this->speed.z, 0.f, 0.0005f);
	}

	//モデルデータ更新
	dx.SetPositionModel(this->hnd, this->pos);
	dx.SetRotateModel(this->hnd, this->mv_ang);
}
...
void Ball_data::SetPos(vec _pos){
	this->pos = _pos;
}
